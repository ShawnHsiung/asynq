...
### Added
- `PreEnqueueFunc`, `PostEnqueueFunc` is added in `Scheduler` and deprecated `EnqueueErrorHandler` (PR: https://github.com/ShawnHsiung/asynq/pull/476)
...
- Fixed overflow issue with 32-bit systems (For details, see https://github.com/ShawnHsiung/asynq/pull/426)
...
- Fixed latency issue around memory usage (see https://github.com/ShawnHsiung/asynq/issues/309).
...
- Free unique lock when task is deleted (https://github.com/ShawnHsiung/asynq/issues/275).
...
**IMPORTATNT**: All `Inspector` related code are moved to subpackage "github.com/ShawnHsiung/asynq/inspeq"
...
- `Scheduler` type is added to enable periodic tasks. See the godoc for its APIs and [wiki](https://github.com/ShawnHsiung/asynq/wiki/Periodic-Tasks) for the getting-started guide.
...
**IMPORTANT**: If you are upgrading from a previous version, please install the latest version of the CLI `go get -u github.com/ShawnHsiung/asynq/tools/asynq` and run `asynq migrate` command. No process should be writing to Redis while you run the migration command.
...
- Fixes issue of same tasks processed by more than one worker (https://github.com/ShawnHsiung/asynq/issues/90).

## [0.9.3] - 2020-06-12

### Fixed

- Fixes the JSON number overflow issue (https://github.com/ShawnHsiung/asynq/issues/166).
...
- [Fixed cancelfunc leak](https://github.com/ShawnHsiung/asynq/pull/145)
